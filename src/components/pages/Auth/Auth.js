...
const Authentication = ({ onLoginSuccess }) => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      console.log("Payload of Auth: ",payload);
  ...
  //     setErrors(prev => ({ ...prev, emailPassword: 'Please enter your email password' }));
  //     return;
  //   }
  ...
  //   try {
  //     const response = await fetch(`${emailServiceUrl}/service/pure_auth`, {
  //       method: 'POST',
  //       headers: {
  //         'Content-Type': 'application/json',
  ...
  //         ...prev, 
  //         email: `Email verified successfully! Welcome ${data.user_info?.username || 'User'}!` 
  //       }));
  //       // Clear any email verification errors
  //       setErrors(prev => {
  //         const newErrors = { ...prev };
  ...
  //       setVerificationStatus(prev => ({ ...prev, email: errorMessage }));
  //     }
  //   } catch (error) {
  //     console.error('Email verification error:', error);
  //     setVerificationStatus(prev => ({ ...prev, email: 'Network error. Please try again.' }));
  ...
  try {
    // ✅ Call Django proxy instead of external service
    const response = await fetch(`http://localhost:8000/trip_package/proxy_email_verification`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
      },
    ...
    const data = await response.json();
      console.log('Proxy email verification response:', data);
      ...
      console.error('Proxy email verification error:', error);
      setVerificationStatus(prev => ({ ...prev, email: 'Network error. Please try again.' }));
      setIsEmailVerified(false);
    } finally {
      setIsVerifyingEmail(false);
          ...
          <div className="form-content">
              ...
              <div className="form-group">
                ...
                <div className="input-container">
                  ...
                  <input
                    ...
                    onChange={handleInputChange}
                    className={`form-input input-with-icon input-with-toggle ${errors.emailPassword ? 'error' : ''}`}
                ...
                {errors.emailPassword && <span className="error-text">{errors.emailPassword}</span>}
            ...
            {!isLogin && (
              <div className="form-group">
                ...
                <OTPFlow
                  secretKey="9D941AF69FAA5E041172D29A8B459BB4"
                  apiEndpoint={`${backendUrl}/trip_package/proxy_check_otp`}
                  initialTheme="light" // or "dark"
                  onComplete={(data) => {
                    console.log("Flow update:", data);
                    if (data.stage === 'verified') {
